...
${incompleteItems.map((item: string) => `  <li style="margin-bottom: 8px; color: #333;">${item}</li>`).join('\n')}
    ...
    try {
      ...
      console.log('📧 Sending email with params:', {
        ...
        nominator_name: templateParams.nominator_name,
        category_name: templateParams.category_name,
        serviceId: EMAILJS_SERVICE_ID,
        templateId: EMAILJS_TEMPLATE_ID,
        hasHtmlContent: !!templateParams.incomplete_items_html,
      });
      ...
      console.log(`✓ Email sent to ${nomineeEmail}`, { status: response.status });
